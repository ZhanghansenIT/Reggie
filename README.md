...
１．在前端展示页面，修改 `选择规格`, 展示分类菜品的规格信息　[香辣，甜辣]
２．在前端展示页面，修改套餐菜品的规格信息同菜品

#### 购物车

**需求分析**

移动端用户可以将菜品或者套餐添加到购物车.对于菜品来说,如果设置了口味信息,则需要
选择规格后才能加入购物车;对于套餐来说,可以直接点击 `+`将当前的套餐加入购物车
  </br>
  
![图片](src/main/resources/images/ca.png)

**代码开发**
1. 点击 `加入购物车`或者 `+`,页面发送ajax请求,请求服务端,将菜品或者套餐添加到
购物车
2. 点击购物车的图标,页面发送ajax请求,请求服务查询购物车中的菜品和套餐
3. 点击购物车按钮,页面发送ajax请求,请求服务端来执行清空购物车操作
准备工作
- 实体类 ShoppingCart
- Maper接口 ShoppingCartMapper
...
需求分析
移动端用户将菜品或者套餐加入购物车之后,可以点击购物车中的
`去结算`的按钮,页面跳转到订单确认页面,点击`去支付` 按钮完成
下单操作


代码梳理
1. 在购物车中点击 ``去结算``,页面跳转到订单确认页面
